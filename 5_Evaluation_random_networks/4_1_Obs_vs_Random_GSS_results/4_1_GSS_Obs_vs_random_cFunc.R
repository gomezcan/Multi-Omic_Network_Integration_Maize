...
# random obs
DFGO <- fread("Input_Random_cFunction.txt")
...
Getgss_cfunc <- function(tf){
  # 
  # # GOs from random nets:  Common targets
  # random_ctarg <- subset(DFGO, TF==tf) 
  # random_ctarg$.id <- as.character(random_ctarg$.id)
  # random_ctarg <- split(random_ctarg$GO.ID, random_ctarg$.id)
  # 
  # # GOs from random nets:  Common targets
  random_cfunct <- subset(DFGO, TF==tf) # # Input_Random_cTarget.txt
  if(nrow(random_cfunct) == 0){
  ...
  gssvals <- lapply(random_cfunct, function(x) gss(ObsGOs_cfunc, x))
  ...
  gssvals[,'TF'] <- tf
  gssvals[,'Method'] <- 'Com.Function'
  
  fwrite(gssvals, paste0("GSSrDB/GSSr_cFunctionn_", tf,'.txt'), row.names = F, quote = F, sep = '\t')
  
  return(cat(paste0(' .. Done TF: ', tf, " ..\n"))) 
  }
}

lapply(AllTFs, Getgss_cfunc)