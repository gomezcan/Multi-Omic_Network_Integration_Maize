...
GetGO <- function(degs, mutant, netindex){
  ...
  GOsname <-   paste0("BP_rNet.CoExp.", netindex,'.GOs.', mutant, ".txt")
  
  # GOs results
  write.table(Res_DF_BP, paste0("RandomNetsData/CoExp_GOs/", GOsname), quote = F, row.names=F, sep = '\t')
...
# CoExp
CoExp <- unique(as_tibble(unique(fread("../Fig_Coexpression/CoExp_NetworkFinal.10_11_2021.txt")))[,c(2,3)])
colnames(CoExp)[1] <- "Source"
...
NetTFs_igraph <- graph_from_data_frame(CoExp, directed = T) 
...
RamdonSample <- function(times){
  ...
  write.table(Rnet, paste0("RandomNetsData/CoExp_Nets/Random_CoExp.",times,".txt"), sep = '\t', row.names = F, quote = F)
