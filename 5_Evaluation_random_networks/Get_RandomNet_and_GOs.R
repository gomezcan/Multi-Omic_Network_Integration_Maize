...
GetGO <- function(degs, mutant, netindex){
  ...
  GOsname <-   paste0("BP_rNet.", netindex,'.GOs.', mutant, ".txt")
  
  # GOs results
  write.table(Res_DF_BP, paste0("RandomNetsData/PDI_GOs/", GOsname), quote = F, row.names=F, sep = '\t')
...
NetTFs_igraph <- graph_from_data_frame(PDI, directed = T) 
...
RamdonSample <- function(times){
  ...
  write.table(Rnet, paste0("RandomNetsData/PDI_Nets/Random_PDI.",times,".txt"), sep = '\t', row.names = F, quote = F)
