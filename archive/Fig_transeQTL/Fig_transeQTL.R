...
trans.eQTL <- subset(trans.eQTL, Target %in% Syntenic & source %in% Syntenic)

# trans-eQTLp
trans.eQTLp <- as_tibble(read.table("Clean_trans.eQTLp.txt", stringsAsFactors = F, h=T))
trans.eQTLp <- subset(trans.eQTLp, Target %in% Syntenic & source %in% Syntenic)

# transcis-eQTL
transcis_eQTL <- as_tibble(read.table("Clean_transcis_eQTL.txt", stringsAsFactors = F, h=T))
transcis_eQTL <- subset(transcis_eQTL, Target %in% Syntenic & source %in% Syntenic)

# cis-eQTL
cis.eQTL <- as_tibble(read.table("Clean_cis.eQTL.txt", stringsAsFactors = F, h=T))
cis.eQTL <- subset(cis.eQTL, Target %in% Syntenic & source %in% Syntenic)

# cis-eQTLt
cis.eQTLt <- as_tibble(read.table("Clean_cis.eQTLt.txt", stringsAsFactors = F, h=T))
cis.eQTLt <- subset(cis.eQTLt, Target %in% Syntenic & source %in% Syntenic)

length(unique(trans.eQTL$Index)) +
  length(unique(trans.eQTLp$Index)) +
  length(unique(transcis_eQTL$Index)) +
  length(unique(cis.eQTL$Index)) +
  length(unique(cis.eQTLt$Index))
  
length(unique(c(unique(trans.eQTL$Target), 
         unique(trans.eQTLp$Target),
         unique(transcis_eQTL$Target),
         unique(cis.eQTL$Target),
         unique(cis.eQTLt$Target),
         unique(eQTLs$Target))))

#
trans.eQTL_full <- rbind(trans.eQTL, trans.eQTLp)

dim(unique(trans.eQTL_full[,c("Target", "source","Index")]))
dim(unique(transcis_eQTL[,c("Target", "source","Index")]))
dim(unique(cis.eQTLt[,c("Target", "source","Index")]))
dim(unique(cis.eQTL[,c("Target", "source","Index")]))
...
net_teQTL <- unique(trans.eQTL_full[,c("source", "Target")])
...
Hisplot_degree.teQTL <- ggplot(Degree_teQTL, aes(x=Targets, fill=Class)) +
  geom_histogram(alpha= 0.5, bins = 35) +
...
temFre_tQTL <- as.data.frame(table(unique(Degree_teQTL[,c(1,3)])$Class))

Hisplot_degree.teQTL_bar <- ggplot(temFre_tQTL, aes(x="", y=Freq, fill=Var1)) +
  geom_bar(position="fill", stat="identity", alpha= 0.5) +
  geom_label_repel(aes(label=scales::comma(Freq), fill=Var1), 
