...
###################################################
# ClusterONE and Pecanpy clusters : V1            #
# nW10 S5                                         #
# nW10, nodes_syntenic, S5, and -d 0.1 --fluff    #
###################################################
...
Clusters <- fread('DistanceCalculation/InputClusterONE_Dim50_WL80_nW10_0.005_syntenic.txt')
Clusters  <- subset(Clusters, V2 %in% Syntenic)
...
SuperGO_Modules_Targ('51')
SuperGO_Modules_Targ('147')
...
test1 <- subset(InputCONE, V1=='Zm00001d005016')$V2
test2 <- as.data.table(table(subset(Clusters, .id %in% c("147"))$V1))$V1
test3 <- subset(FullNet, Source=='Zm00001d005016')$Target

venn(list(test1=test1,
          test2=test2,
          test3=test3))
...
SuperGO_Modules_Targ('376')

subset(Clusters, V1=="Zm00001d033859") # KN1
subset(Clusters, V1=="Zm00001d051520") # MYB19

subset(Clusters,  .id=="48") # MYB31
subset(Clusters,  .id=="416") # MYB31
subset(Clusters,  .id=="160") # WRI1
...
View(subset(GOs_ClustersFiltered, Module %in% subset(Clusters, V1=="Zm00001d006236")$.id))
View(subset(GOs_ClustersFiltered, Module %in% subset(Clusters, V1=="Zm00001d005016")$.id))

subset(Clusters, V1 =="Zm00001d006236") # MYB31
subset(Clusters, V1 =="Zm00001d005016") # WRI1

table(GOs_ClustersFiltered$Module)
