...
CornCYC_list <- split(CornCYC$GeneID, CornCYC$Pathway.id)
##################################################
...
# list of TFs in all three networks
All_TFs <- unique(c(PDI$Source, CoExp$Source, TF_CoR$GeneID))

# Total TFs by layer
Fullnets <- unique(rbind(CoExp, PDI, teQTL))
write.table(Fullnets, "../Fig_pecanpy/uniqFullNets.10_11_2021.txt", row.names = F, col.names = F, quote = F, sep = "\t")


Fullnets <- subset(Fullnets, Source %in% All_TFs)

CornCYC_size[,"PWYg"] <- CornCYC_size$PWYSize/length(Syntenic)

Fullnets <- split(Fullnets$Target, Fullnets$Source)

# Total targets by TF
Fullnets_s <- as.data.frame(mapply(length, Fullnets))
Fullnets_s[,"TF"] <- row.names(Fullnets_s)
Fullnets_s <- as_tibble(Fullnets_s)
colnames(Fullnets_s)[1] <- "Ttarg"


EnrScore <- function(targets_list){
  # used list of target to get enrichment scole by pwy
  
  print(". Pre-newGOM .")
  # GeneOverlap::newGOM 
  go.obj <- newGOM(targets_list, CornCYC_list, genome.size=length(Syntenic)) # annotated genes in Genome v4
  #go.obj <- newGOM(Fullnets, CornCYC_list, genome.size=length(Syntenic)) # annotated genes in Genome v4
  
  # number of Common genes 
  Common <- getMatrix(go.obj, name="intersection")
  
  # as df 
  Common <- t(as.data.frame(Common))
  
  Common   <- as_tibble(reshape2::melt(as.matrix(Common)))
  colnames(Common) <- c('PWY', 'TF', 'Common')
  Common <- left_join(Common, Fullnets_s, by="TF")
  
  Common[,"TargP"] <- Common$Common/Common$Ttarg
  #
  Common <- left_join(Common, CornCYC_size[,c(1,3)], by="PWY")
  
  # ES: frat_targets_in_pwy/frag_pWY
  Common[,"ES"] <- log2(Common$TargP/Common$PWYg)
  
  M <- as_tibble(as.data.frame(reshape2::dcast(Common[,c(1,2,7)],  PWY ~ TF)) )
  
  M[M == -Inf] <- 0 # -inf to zero

  M1 <- M[,2:ncol(M)] # to scale by columns
  M2 <- M[,2:ncol(M)] # to scale by rows
  
  M1[, 1:ncol(M1)] <- apply(M1, 2, scale)
  M2[, 1:ncol(M2)] <- t(apply(M2, 1, scale))
  ...
  Mf_ranks <- t(apply(-replace(Mf,Mf==0, min(Mf)), 1, rank, na='keep'))
  
  Mf_DF <- as_tibble(reshape2::melt(as.matrix(Mf)))
  Mf_ranksDF <- as_tibble(reshape2::melt(as.matrix(Mf_ranks)))
  
  
  colnames(Mf_DF) <- c("PWY", "TF", "Z")
  colnames(Mf_ranksDF) <- c("PWY", "TF", "Zrank")
  
  Common <- left_join(Common, Mf_DF, by=c("PWY", "TF"))
  Common <- left_join(Common, Mf_ranksDF, by=c("PWY", "TF"))
  
  
  return(list(ES=Mf, ESrank=Mf_ranks, DF=Common))

}

ES_obj <-  EnrScore(Fullnets)

write.table(ES_obj$ES, "ES_Full_matrix.txt", sep = "\t", quote = F)
write.table(ES_obj$ESrank, "ESrank_Full_matrix.txt", sep = "\t", quote = F)
write.table(ES_obj$DF, "ES_Results.txt", sep = "\t", quote = F, row.names = F)



MakeHeatmap <- function(df_ES, df_Rank) {
  
  df_ES <- as.matrix(df_ES)
  df_Rank <- as.matrix(df_Rank)
  
  # add PWY name
  rownames(df_ES) <- ReplaceNamePWY(rownames(df_ES))
  # add TFs names
  colnames(df_ES) <- ReplaceName(colnames(df_ES))
  
  rownames(df_Rank) <- ReplaceNamePWY(rownames(df_Rank))
  colnames(df_Rank) <- ReplaceName(colnames(df_Rank))

  df_Rank <- (df_Rank)
  df_ES <- (df_ES)
  
  #cor_scale <- colorRamp2(seq(0,10,1), viridis(11, direction = -1, option = "A"))
  
  df_ES[df_ES >= 20] <- 20
    
  hm_common <- Heatmap(df_ES, 
                       use_raster = TRUE,
                       ...
                       #   if(df_Rank[i, j]>0){
                       #     grid.text(sprintf("%.f", df_Rank[i, j]), x, y, gp = gpar(fontsize = 7, col = "gray70"))
                       #   }
                       #   
                       # },
                       name = "Z_es", 
                       ...
                       col=viridis(100, direction = -1, option = "A"), #cor_scale, 
...
heatmap_CLR <- MakeHeatmap(ES_obj$ES, ES_obj$ESrank)
draw(heatmap_CLR, heatmap_legend_side = "bottom")
...
View(subset(Total_TF_PWY, TF =="MYB31"))
