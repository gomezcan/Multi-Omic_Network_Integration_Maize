...
# Only TFs
teQTL <- subset(teQTL, Source %in% TF_CoR$GeneID)
...
#print(PDI[1:10,])
# length(subset(teQTL_counts, Source %in% TF_CoR$GeneID & Targets >= 10)$Targets)
TFs <- unique(subset(teQTL_counts, Source %in% TF_CoR$GeneID & Targets >= 10)$Source)
...
for (t in TFs){
  tgs <- as.character(subset(teQTL, Source==t)$Target)
  #
  GetGO(tgs, t, "teQTL")
}
